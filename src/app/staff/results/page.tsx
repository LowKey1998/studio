...
export default function StaffResultsPage() {
    ...
    return (
        <div className="space-y-6">
            <Card><CardHeader><CardTitle className="font-headline text-2xl">Enter Results</CardTitle><CardDescription>Select a course to record student grades.</CardDescription></CardHeader></Card>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {courses.map(course => (
                    <Card key={course.id}>
                        ...
                        <CardFooter><Button asChild className="w-full"><Link href={`/staff/courses/${course.id}/assessment`}><ClipboardEdit className="mr-2 h-4 w-4" /> Enter Results</Link></Button></CardFooter>
                    </Card>
                ))}
            </div>}
