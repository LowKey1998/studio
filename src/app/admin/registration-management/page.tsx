...
export default function RegistrationManagementPage() {
    ...
    const [selectedProgramme, setSelectedProgramme] = React.useState<string>('all');
    ...
    const refreshData = React.useCallback(async () => {
        ...
        try {
            ...
            const semesterOfferingsData = semesterOfferingsSnap.exists() ? semesterOfferingsSnap.val() : {};
            ...
            setActivePathSemesters(semesterOfferingsData);
    ...
    const filteredIntakes = React.useMemo(() => {
        if(selectedProgramme === 'all') return allIntakes;
        const intakeIdsWithProgramme = new Set(allCoursePaths.filter(p => p.programmeId === selectedProgramme).map(p => p.intakeId));
        return allIntakes.filter(i => intakeIdsWithProgramme.has(i.id));
    }, [allIntakes, allCoursePaths, selectedProgramme]);
            ...
            <CardHeader>
                ...
                <div className="flex gap-4 pt-4">
                     <div className="flex-1">
                        ...
                        <Select value={selectedProgramme} onValueChange={setSelectedProgramme}><SelectTrigger id="programme-filter"><SelectValue placeholder="All Programmes"/></SelectTrigger><SelectContent><SelectItem value="all">All Programmes</SelectItem>{allProgrammes.map(p => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}</SelectContent></Select>
                    </div>
                    <div className="flex-1">
                        <Label htmlFor="intake-filter">Filter by Intake</Label>
                        <Select value={selectedIntake} onValueChange={setSelectedIntake}><SelectTrigger id="intake-filter"><SelectValue placeholder="All Intakes"/></SelectTrigger><SelectContent><SelectItem value="all">All Intakes</SelectItem>{filteredIntakes.map(i => <SelectItem key={i.id} value={i.id}>{i.name}</SelectItem>)}</SelectContent></Select>
                    </div>
                </div>
            </CardHeader>
            <CardContent>
                           ...
                           {filteredIntakes.filter(i => selectedIntake === 'all' || i.id === selectedIntake).map(intake => (
                                <AccordionItem value={intake.id} key={intake.id}>
                                    ...
                                    <AccordionContent className="space-y-4">
                                        {allProgrammes.filter(p => selectedProgramme === 'all' || p.id === selectedProgramme).map(programme => {
