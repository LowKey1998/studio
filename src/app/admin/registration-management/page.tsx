...
type PaymentPlan = { id: string; name: string; installments: number; installmentPercentages: number[]; };
...
export default function RegistrationManagementPage() {
    ...
    React.useEffect(() => {
        ...
        const unsubs = refs.map((r, i) => onValue(r, (snapshot) => {
            ...
            switch(i) {
                ...
                case 6: setAllSemesters(Object.keys(data).map(id => ({ id, ...data[id] })).sort((a,b) => b.name.localeCompare(a.name))); break;
            }
        }));
    ...
    const handleOpenDeadlineDialog = async (semesterName: string) => {
        ...
        const semesterData = allSemesters.find(s => s.name === semesterName);
