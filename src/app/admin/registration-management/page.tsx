...
export default function RegistrationManagementPage() {
    ...
    const fetchSemesters = React.useCallback(() => {
        ...
        return onValue(semestersRef, (snapshot) => {
            if (snapshot.exists()) {
    ...
    React.useEffect(() => {
        ...
        const unsubs = refs.map((r, i) => onValue(r, (snapshot) => {
            const data = snapshot.val() || {};
            switch(i) {
                case 0: setAllIntakes(Object.keys(data).map(id => ({ id, ...data[id] }))); break;
                ...
                case 6: setFeeTemplates(Object.keys(data).map(id => ({ id, ...data[id] }))); break;
            }
        }));
        
        const unsubSemesters = fetchSemesters();
        setLoading(false);
        return () => { unsubs.forEach(unsub => unsub()); unsubSemesters(); };
    }, [fetchSemesters]);
