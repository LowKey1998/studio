...
export default function ResearchCalendarPage() {
  ...
  React.useEffect(() => {
    const eventsRef = ref(db, 'researchEvents');
    const unsub = onValue(eventsRef, (snapshot) => {
        const eventsList: ResearchEvent[] = [];
        if (snapshot.exists()) {
  ...
  const handleAddEvent = async (e: React.FormEvent) => {
    ...
    setFormLoading(true);
    try {
      const newEventRef = push(ref(db, 'researchEvents'));
    ...
    <div className="space-y-6">
      <Card className="shadow-lg">
        <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <CardTitle className="font-headline text-2xl">Research Calendar</CardTitle>
            <CardDescription>Schedule and view important research milestones and deadlines.</CardDescription>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={(isOpen) => { setIsDialogOpen(isOpen); if(!isOpen) resetForm(); }}>
              ...
              <DialogContent className="sm:max-w-md">
                <form onSubmit={handleAddEvent}>
                  <DialogHeader><DialogTitle className="font-headline">Add New Research Event</DialogTitle></DialogHeader>
                  <div className="grid gap-4 py-4">
                    <div className="space-y-1">
                        <Input placeholder="e.g., Thesis Proposal Deadline" value={newEvent.title} onChange={e => setNewEvent({...newEvent, title: e.target.value})} disabled={formLoading} />
                    </div>
                    <div className="space-y-1">
        ...
        <CardContent className="flex flex-col gap-6 md:flex-row">
          ...
          <div className="flex-1 space-y-4">
             <h3 className="font-headline text-lg font-semibold">Upcoming Deadlines & Milestones</h3>
